...
> ## ⚡ P31 갱신 — 항상 다국어 OCR (2026-07-05, v0.1.16)
> **전세계 매칭 대응:** 상대 이름이 어느 언어로 떠도 읽도록, ML Kit 스크립트별 인식기 **4개(Latin/Korean/Japanese/Chinese)를 항상 병렬**로 돌린다(captureLang 설정 제거). 앱 **표시** 언어(displayLang)만 유저가 고른다.
>
> ### 실기기 확인 항목 (P31)
> 1. **설정에 "게임 화면 언어(인식용)" 항목이 없는지** — 표시 언어 선택만 남아야.
> 2. **다국어 인식**: 상대가 **한국어/영어/일본어/중국어/유럽어(독·스·불·이)** 어느 언어로 떠도 카드가 뜨는지. (에뮬 실측: 한국어 `나인테일/파이어로`, 영어 `hippowdon/sylveon` 모두 captureLang 없이 인식 — `field_samples/p31/`.)
> 3. **지연/발열(중요)**: 4개 인식기를 상시 병행하므로 실기기 장시간 세션에서 **지연 증가·발열**이 실용 범위인지 확인. 진단 스트립 "OCR n회/s" 와 매칭 스크립트 태그(`[L]`/`[K]`)로 어떤 스크립트로 읽혔는지 확인 가능. (에뮬 SW-GPU 절대값은 수 초로 크지만 실기기 HW-GPU/NNAPI 는 크게 낮음 — 실기기 재확인 잔여.)
> 4. **오탐 없음**: 엉뚱한 스크립트 인식기가 만든 garbage 로 잘못된 카드가 뜨지 않는지(짧은 이름 fuzzy 억제 + 닫힌 사전 필터).

> ## ⚡ P30 갱신 — 확장 카드 가로 2컬럼 + 모서리 드래그 리사이즈 (2026-07-05, v0.1.15)
> 가로 화면에서 확장(EXPANDED) 카드가 세로로 길어 아래가 잘리던 문제를 해결했다.
> EXPANDED 를 **가로 2컬럼**(왼쪽 특성/주요기술, 오른쪽 종족값 가로 한 줄 + 방어 상성)으로 재구성하고,
> **한 번에 한 카드만 EXPANDED**(다른 슬롯 자동 CARD 축소) + 넘칠 때만 내부 스크롤(화면 ~85% 상한)을 넣었다.
> 또 카드 **우하단 그립 드래그**로 크기를 0.6~2.0 연속 조절(즉시 반영·영속). 설정 칩(80/100/125/150%)/리셋도 유지.
>
> ### 실기기 확인 항목 (P30)
> 1. **EXPANDED 2컬럼 잘림 해소(핵심)**: 실 대전/데모에서 카드를 확장했을 때 종족값·기술·상성이
>    **가로 2컬럼**으로 뜨고 **화면 아래가 잘리지 않는지**. 종족값은 **H·A·B·C·D·S 가로 한 줄**(값 줄바꿈 없음).
>    (에뮬 실측 1280x720: Garchomp 확장 카드가 왼쪽 특성/기술·오른쪽 종족값+상성으로 전부 화면 안에 표시 —
>     `field_samples/p30/02_after_expanded_2col.png`.)
> 2. **모서리 드래그 리사이즈**: 조작 모드에서 카드 우하단 그립을 바깥으로/안으로 드래그 →
>    카드가 **연속적으로** 커지고/작아지는지, 재실행 후에도 크기가 유지되는지(영속).
>    (에뮬 실측: 그립 드래그로 `overlay_scale` 1.0→1.279→0.935 변화 + 창 frame 확대/축소 확인 —
>     `03_resize_grow_128pct.png` / `04_resize_shrink_94pct.png`.)
> 3. **한 번에 하나만 EXPANDED(더블)**: 더블배틀에서 한 카드를 확장하면 다른 카드가 CARD 로 축소되어
>    둘 다 세로로 길어지지 않는지. (에뮬 데모는 단일 슬롯이라 시각확인 불가 → `ExpandExclusiveTest`(JVM)로 검증.)
> 4. **터치 통과 보존**: 넓어진 카드 위 탭도 통과 모드(🔒)에선 게임으로 통과하는지(P24 유지).
> 5. **설정 칩/리셋 유지**: 설정의 80/100/125/150% 칩과 리셋이 여전히 동작하는지(칩=특정값, 드래그=미세조정).
>
> **versionCode 17 / versionName 0.1.15**. release 데이터 URL 불변.
