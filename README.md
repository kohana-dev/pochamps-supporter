...
언어 개념이 둘이고, **설정에서 각각 따로** 고른다.

| 개념 | 설정 항목 | 저장키 | 무엇을 좌우하나 |
|---|---|---|---|
| **게임 화면 언어** (캡처/OCR) | "게임 화면 언어(인식용)" | `AppSettings.language` | OCR recognizer + 이름 매칭. 게임이 상대 이름을 띄우는 언어에 맞춘다(기술적 필수). |
| **앱 표시 언어** (UI + 카드) | "앱 표시 언어" | `AppSettings.displayLang` | 앱 UI 문자열 + 카드 내용(포켓몬 이름·타입·특성·기술). 유저가 보고 싶은 언어. |

- **왜 분리하나**: OCR 이 어떤 언어를 읽든 → `matchBest` 가 **도감키**를 확정(언어 무관) → 카드는 **표시 언어**로 렌더. 즉 게임이 일본어여도 오버레이를 한국어로 볼 수 있다(우리 DB 가 도감번호로 9언어를 매핑하므로 자연스럽다).
- **앱 UI 9개 언어**: `values`(ko 기본)·`-en`·`-ja`·`-de`·`-es`·`-fr`·`-it`·`-zh-rCN`·`-zh-rTW`. **품질**: ko/ja/en 정확, 유럽어(de/es/fr/it)·중국어(간체/번체)는 best-effort(각 파일 상단에 원어민 검수 권장 주석). 도메인 용어(타입/특성/기술)는 카드가 데이터 사전의 정확한 9언어를 쓰므로 UI chrome 은 일반 용어 위주.
- **표시 언어 오버라이드**: `LocaleUtils.wrap()` 으로 `MainActivity`·`CaptureService` 의 `attachBaseContext` 를 표시 언어 로케일로 래핑 → Compose 와 오버레이/서비스 문자열이 모두 표시 언어로 해석(시스템 로케일 불변). 변경 즉시(`recreate()`) 반영, 오버레이는 다음 세션부터.
- **기본 표시 언어**: 시스템 로케일을 9언어 중 가장 가까운 것으로 매핑(중국어는 Hant/TW/HK/MO→zh-tw, 그 외 zh-cn; 미지원은 en).
